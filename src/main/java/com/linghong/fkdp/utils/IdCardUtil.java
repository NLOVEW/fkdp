...
public class IdCardUtil {
    ...
    public static boolean idCardValidate(String IDStr) {
        IDStr = IDStr.toUpperCase();//将最后一位x转为大写
        String Ai = "";
        if (IDStr.length() != 15 && IDStr.length() != 18) {//身份证号码长度应该为15位或18位
            logger.error("!!!身份证号码长度应该为15位或18位!!!");
            return false;
        }
        if (IDStr.length() == 18) {//18位身份证前17位位数字，如果是15位的身份证则所有号码都为数字
            Ai = IDStr.substring(0, 17);
        } else if (IDStr.length() == 15) {
            Ai = IDStr.substring(0, 6) + "19" + IDStr.substring(6, 15);
        }
        if (isNumeric(Ai) == false) {//身份证15位号码都应为数字 ; 18位号码除最后一位外，都应为数字
        ...
        if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {//身份证出生日期无效
        ...
        try {
            if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
                    ...
                    strYear + "-" + strMonth + "-" + strDay).getTime()) < 0) {//身份证生日不在有效范围
        ...
        if (areaCode.get(Ai.substring(0, 2)) == null) {//身份证地区编码错误
            logger.error("身份证地区编码错误");
            return false;
        }
        if (isVarifyCode(Ai, IDStr) == false) {//身份证校验码无效，不是合法的身份证号码
    ...
    /**
     * 判断字符串出生日期是否符合正则表达式：
     * 包括年月日，闰年、平年和每月31天、30天和闰月的28天或者29天
     *
     * @param strDate
     * @return
     */
